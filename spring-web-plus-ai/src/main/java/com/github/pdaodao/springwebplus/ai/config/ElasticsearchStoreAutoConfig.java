...
public class ElasticsearchStoreAutoConfig {
    ...
    @Bean
    public AiVectorStore aiVectorStore(final ElasticsearchOptions options,
                                       final ElasticsearchClientService clientService, final AiEmbeddingAutoConfig embeddingAutoConfig) throws Exception {
        final ElasticsearchVectorStore store = new ElasticsearchVectorStore(clientService.getClient(), options, embeddingAutoConfig.getDimension());
        return store;
    }
}
