...
public class DynamicWhereTest {
    ...
    public static void main2(String[] args) throws Exception {
        final String sql = "select * from t1 where a = #{a} and ${aa} = #{ab} " +
                "if{b > 1, and b = #b and c > #{c} and d < #d }  and f1(#{e}) > 3 and f is not null order by a desc";
    ...
    public static void main(String[] args) throws Exception {
        final String sql = "select * from t1 where a = #{a}\n" +
                "<if test=\"b\">\n" +
                "and b = #{b} and c = 'c'\n" +
                "</if>";
        final List<FilterItem> fs = new ArrayList<>();
