...
public class DBDdLUtil {
    ...
    public static String autoIdRestart(final DbDialect dbDialect, final DataSource ds, final TableInfo tableInfo) {
        final TableField autoIdColumn = tableInfo.autoIdColumn();
    ...
    public static List<String> genDDL(final DbDialect dbDialect,
        ...
        // 对比上次表结构同步后的变化
        final CaseInsensitiveLinkedMap<String, TableField> fieldMap = new CaseInsensitiveLinkedMap();
        final CaseInsensitiveLinkedMap<String, TableField> oldFieldMap = new CaseInsensitiveLinkedMap();
        ...
        for (final TableField f : old.getFields()) {
            ...
            // 字段结构发生变化 更新
            final TableField toUpdated = fieldMap.get(f.getName());
